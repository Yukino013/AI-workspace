...
export async function streamTool(userId: string, key: string, input: { model: string; code: string; language?: string }, onDelta: (text: string) => void, signal: AbortSignal) {
  ...
  try { for await (const event of AIService.stream({ model: input.model, messages, apiKey: await getApiKeyForModel(userId, input.model) }, signal)) { if (event.type === 'delta') { output += event.content; onDelta(event.content); } else usage = event.usage; } await CodeToolRecord.create({ userId, toolKey: key, title: `${tool.name} · ${input.code.split('\n')[0].slice(0, 40)}`, model: input.model, input: input.code, output, language: input.language ?? '', duration: Date.now() - start, tokenUsage: usage, status: signal.aborted ? 'aborted' : 'success' }); } catch (err) { const aborted = signal.aborted || (err instanceof Error && err.name === 'AbortError'); await CodeToolRecord.create({ userId, toolKey: key, title: tool.name, model: input.model, input: input.code, output, language: input.language ?? '', duration: Date.now() - start, status: aborted ? 'aborted' : 'error', errorMessage: aborted ? undefined : err instanceof Error ? err.message : 'AI 调用失败' }); if (!aborted) throw err; }
}
